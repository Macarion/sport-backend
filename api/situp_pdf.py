...
class SitupPDFGenerator:
    ...
    def __init__(self):
        ...
        # 配色方案 - 绿色系
        self.primary_color = HexColor("#2E7D32")      # 深绿
        self.secondary_color = HexColor("#4CAF50")    # 中绿
        self.light_color = HexColor("#C8E6C9")        # 浅绿
        self.accent_color = HexColor("#FF9800")       # 橙色强调
        self.bg_color = HexColor("#F1F8E9")           # 背景绿
    ...
    def draw_header_banner(self):
        ...
        self.pdf_canvas.drawCentredString(self.page_width / 2, self.page_height - 40, "仰卧起坐 单项成绩报告")
    ...
    def draw_score_gauge(self, score, level):
        ...
        # 保存图片
        gauge_path = 'temp_gauge_situp.png'
    ...
    def draw_standard_table(self):
        ...
        self.pdf_canvas.drawString(50, self.y, "📊 评分标准")
        ...
        data = [
            ["优秀", "54次及以上", "90-100分", "核心力量出色"],
            ["良好", "42-53次", "70-89分", "表现较好"],
            ["良", "36-41次", "60-69分", "达到中等水平"],
            ["合格", "30-35次", "50-59分", "达到基本要求"],
            ["不合格", "30次以下", "50分以下", "需要加强锻炼"],
        ]
    ...
    def draw_suggestion_box(self, remark, score, level):
        ...
        # 建议框背景
        self.pdf_canvas.setFillColor(HexColor("#FFF8E1"))  # 淡黄色背景
        ...
        if level == "优秀":
            tips = [
                f"当前评价：{remark}",
                "• 继续保持，可尝试负重仰卧起坐或增加训练难度",
                "• 建议每周训练3-4次，每次3-4组，保持高水平状态",
                "• 可作为同学的训练榜样，帮助他人提升"
            ]
        elif level == "良好":
            tips = [
                f"当前评价：{remark}",
                "• 距离优秀仅一步之遥，增加每组次数冲刺满分",
                "• 建议每天练习2-3组，每组25-30个，提升耐力",
                "• 配合平板支撑训练，强化核心稳定性"
            ]
        elif level == "良":
            tips = [
    ...
    def get_score_and_remark(self, count):
        ...
        if percentage < 50:
            score = percentage
            level = "不合格"
            remark = "需要加强锻炼，建议每天练习仰卧起坐"
        elif 50 <= percentage < 60:
            score = percentage
            level = "合格"
            remark = "达到基本要求，继续保持并适当增加训练量"
        elif 60 <= percentage < 70:
            score = percentage
            level = "良"
            remark = "表现不错，有一定基础，可以挑战更高目标"
        elif 70 <= percentage < 90:
            score = percentage
            level = "良好"
            remark = "较为优秀，核心力量较强，保持稳定训练"
        else:
            score = min(100, percentage)
            level = "优秀"
            remark = "非常优秀！核心力量出色，继续保持"
    ...
    def generate_report(self, username, situp_count):
        """
        生成仰卧起坐单项报告（美化版）
        ...
        # 计算成绩
        score, level, remark = self.get_score_and_remark(situp_count)
        ...
        # 4. 成绩结果卡片
        self.draw_result_card(situp_count, score, level)
        ...
        # 保存 PDF
        self.save_pdf("report_pdf/situp_report.pdf")
        
        return "report_pdf/situp_report.pdf"
