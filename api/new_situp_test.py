...
def _detect_marker_center_bgr(frame: np.ndarray) -> Optional[tuple[int, int]]:
    """
...
def process_frame(img, frame_id, WIDTH, HEIGHT, testing):
...
class SITUP():
    ...
    def situp_start(self, frame, WIDTH, HEIGHT):
        ...
        # 检测开始前（self.if_start == 2 之前）：若检测到标志物，用其坐标替换 position[23] 的横纵坐标
        # 未检测到标志物则保持原方法，不替换
        if if_existperson == 1 and isinstance(position, list) and len(position) > 23:
            # if self.if_start != 2 and (1 not in self.IF_START):
                marker_xy = _detect_marker_center_bgr(frame)
                if marker_xy is not None:
                    position[23][1], position[23][2] = marker_xy
